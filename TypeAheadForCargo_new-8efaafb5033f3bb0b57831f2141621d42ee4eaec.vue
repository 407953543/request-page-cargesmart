      ...
      <div>
        <ul v-if="hasItems && !blur && ((minChars && query.length>=minChars) || showRecommend)" class="ul-wrap" ref="ul">
          <li v-for="(item, $item) in items" :class="activeClass($item)" @mousedown="hit" @mousemove="setActive($item)">
            <b v-text="item.undgNumber"></b>
            <span v-text="' '+item.properShippingName+' '+item.imoClass+' '+item.packingGroup"></span>    
          </li>
        </ul>
        <ul v-show="!hasItems && isFocus && (minChars && query.length>=minChars) && !loading && !isAutoComplete"
            class="ul-wrap">
          <li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onHit(item)
      {
        this.query = this.items[this.current].undgNumber;
        this.blur = true;
